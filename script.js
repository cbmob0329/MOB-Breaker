// script.js – BOOST Rev1 (爽快版)
// ・A1ダメ+10
// ・A2ダメ+30 & 超吹っ飛び + 回転
// ・Skill1ダメ+20 & 吹っ飛び大幅UP（回転）
// ・ULT即フル溜め & 巨大化 & 超ぶっ飛び（回転）
// ・Start安定化（ボタン/画面/キー入力すべてで開始）
 ...
 * Tunables
 * ================================ */
const ST1_BG_YOFF = -20; // 入口背景の縦オフセット（必要なら微調整）
const ULT_POWER    = 500; // ULT基礎ダメ
const ULT_BASESIZE = 120; // ULT見た目の基準サイズ（旧60の約2倍）
const STAGE_RIGHT_DEFAULT = 2200;
...
class Effects{
  ...
  addSpark(x,y,strong=false){
    ...
    if(strong){ this.shake(0.16,9); this.hitstop=Math.max(this.hitstop,0.08); }
...
class Input{
  ...
  _initKeyboard(){
    addEventListener('keydown',(e)=>{
      ...
      // タイトル時のフェイルセーフ開始
      if(window.__GAME__?.state==='title'){ window.__GAME__?.beginStage1(); }
    },{passive:false});
  ...
  _initTouch(){
    ...
    bind('btnA1', ()=>{ this.btn.a1=true; if(window.__GAME__?.state==='title') window.__GAME__?.beginStage1(); }, ()=>{ this.btn.a1=false; });
    bind('btnA2', ()=>{ if(!this.btn.a2){ this.btn.a2=true; this.edge.a2Press=true; if(window.__GAME__?.state==='title') window.__GAME__?.beginStage1(); } }, ()=>{ this.btn.a2=false; });
    bind('btnSK', ()=>{ if(!this.btn.skill){ this.btn.skill=true; this.edge.skillPress=true; this.skillCharging=true; this.skillChargeT=0; if(window.__GAME__?.state==='title') window.__GAME__?.beginStage1(); } }, ()=>{ if(this.btn.skill){ this.btn.skill=false; this.edge.skillRelease=true; this.skillCharging=false; } });
    bind('btnSK2', ()=>{ this.edge.skill2=true; this.btn.skill2=true; if(window.__GAME__?.state==='title') window.__GAME__?.beginStage1(); }, ()=>{ this.btn.skill2=false; });
    bind('btnULT', ()=>{ if(!this.btn.ult){ this.btn.ult=true; this.edge.ultPress=true; this.ultCharging=true; this.ultChargeT=0; if(window.__GAME__?.state==='title') window.__GAME__?.beginStage1(); } },
                   ()=>{ if(this.btn.ult){ this.btn.ult=false; this.ultCharging=false; this.edge.ultRelease=true; } });
    bind('btnJMP', ()=>{ this.jump=true; if(window.__GAME__?.state==='title') window.__GAME__?.beginStage1(); }, ()=>{ /* release */ });
  }
...
class CharacterBase{
  constructor(w,h){
    ...
    this.world=null; this._prevY=0; this._spinDecayT=0;
  }
  ...
  hurt(amount, dir, opts={}, effects){
    ...
    // 強ヒットボーナス（更に強化）
    const STRONG_X = isStrong ? 2.6 : 1.0;
    const STRONG_Y = isStrong ? 2.3 : 1.0;

    // ULTは更に上限突破風味
    const extraUltX = isULT ? 1.3 : 1.0;
    const extraUltY = isULT ? 1.2 : 1.0;

    this.vx = clamp(dir * baseKb * kbMulIn * STRONG_X * extraUltX, -900, 900);
    this.vy = - clamp(baseKbu * kbuMulIn * STRONG_Y * extraUltY, 0, 900);

    this.x += dir * (isStrong? 5 : 2);
    this.face = -dir;
    ...
    if(opts.spin || isStrong){
      ...
      this._spinDecayT = Math.max(this._spinDecayT, 0.55);
    }

    if(effects){
      effects.addSpark(this.x, this.y-10, isStrong || amount>=15);
      if(isStrong){ effects.shake(0.2,11); effects.hitstop=Math.max(effects.hitstop,0.11); }
    }

    if(this.hp<=0){
      this.dead=true; this.vx = dir * 560; this.vy = -620; this.spinSpeed = 18; this.deathT = 0; this.fade = 1;
    }
    return true;
  }
...
class UltBlast extends Projectile{
  constructor(world,x,y,dir,img,chargeSec){
    super(world,x,y,dir,img,ULT_POWER);
    const cs = 3.0; // 即フル扱い
    const sizeMul = lerp(0.9, 1.6, clamp(cs/3.0,0,1)); // 最大1.6倍
    this.w = Math.round(ULT_BASESIZE*sizeMul);
    this.h = Math.round(ULT_BASESIZE*sizeMul);
    this.vx = (320 + 140*sizeMul) * dir;
    this.life = 2.2 + 0.4*sizeMul;
  }
}
...
class Player extends CharacterBase{
  ...
  update(dt,input,world,enemies){
    ...
    // ULT：即MAX扱い（ゲージは演出だけ高速満タン）
    this.isUltCharging = input.ultCharging && this.ultCDT<=0;
    if(this.isUltCharging){
      input.ultChargeT = Math.min(3, input.ultChargeT + dt*9); // ほぼ一瞬で満タンに見える
    ...
    if(input.edge.ultRelease && this.ultCDT<=0){
      // 実際は常にMAX（chargeSecは無視 or 3.0固定）
      this._releaseULT(3.0);
    ...
    if(input.edge.skill2 && this.skill2CDT<=0){ input.edge.skill2=false; this._startSkill2(); return; }
  ...
  _startA1(){
    ...
    // ★ +10ダメ
    let frame='k1a', power=16, fx=150;
    if(this.comboStep===2){ frame='k1b'; power=19; fx=180; }
    else if(this.comboStep===3){ frame='k1c'; power=22; fx=210; }
    seq.push({kind:'hit',dur:0.20,frame,fx,power, kbMul:1.05, kbuMul:1.05});
    this._actionSeq=seq; this._actionIndex=0; this._actionTime=0;
  }
  _startA2(){
    this.state='atk'; this.animT=0;
    this._actionSeq=[
      {kind:'prep',dur:0.10,frame:'k2prep',fx:100,power:0},
      // ★ +30ダメ & 超KB & 回転
      {kind:'hit', dur:0.22,frame:'k2',fx:260,power:48, lift:1.0, kbMul:2.6, kbuMul:2.3, spin:true, tag:'smash'}
    ];
  ...
  _releaseULT(_chargeSecIgnored){
    if(this.ultCDT>0) return;
    this.state='ult'; this.animT=0;
    this._actionSeq=[
      {kind:'pose',dur:0.06,frame:'ul2',fx:40},
      {kind:'post',dur:0.18,frame:'ul2',fx:20}
    ];
    this._actionIndex=0; this._actionTime=0;
    this.ultCDT=3.0;

    const img=this.world.assets.img(this.frames.ul3);
    const ox=this.face*32, oy=-12;
  ...
  draw(ctx,world){
    ...
    if(this.isUltCharging){
      ...
      if(ul3){
        ...
        const hh=ULT_BASESIZE*(mul*0.5), ww=ULT_BASESIZE*(mul*0.5);
        const oxh = this.face*26, oyh=-14;
...
class Screw extends CharacterBase{
  constructor(world,effects,assets,x=1500){
    ...
    this.maxhp=2000; this.hp=2000; // ステ1用
  ...
  hurt(amount, dir, opts={}, effects){
    // たまにSAで粘る
    const proc = Math.random()<0.25;
    if(proc){
      opts={...(opts||{}), kbMul:(opts.kbMul??1)*0.45, kbuMul:(opts.kbuMul??1)*0.42, spin:true};
  ...
  update(dt, player){
    ...
    if(this._seq){
      ...
      if(cur?.hit){
        ...
        if(player.invulnT<=0 && rectsOverlap(hb, player.aabb())){
          const hit=player.hurt(cur.power, this.face, {lift:cur.lift,kbMul:cur.kbm,kbuMul:cur.kbum, spin:true}, this.effects);
    ...
    if(this.brainT<=0){
      ...
      if(this.cool<=0){
        if(adx<150 && Math.random()<0.6){
          this.state='atk'; this._seq=[
            {dur:0.10, key:'a1a', fx:160},
            {dur:0.16, key:'a1b', fx:210, hit:true, hx:20, hw:48, hh:38, power:32, lift:0.4, kbm:1.0, kbum:1.0}
          ];
          this.cool=1.1;
          this._idx=0; this._t=0; this.animT=0; return;
        }
        if(adx<330 && Math.random()<0.5){
          this.state='skill';
          this._seq=[
            {dur:0.46, key:'sPrep', fx:0},
            {dur:0.20, key:'s1', fx:560, hit:true, hx:22, hw:56, hh:40, power:55, lift:0.6, kbm:1.05, kbum:1.05},
            {dur:0.14, key:'s2', fx:420, hit:true, hx:20, hw:44, hh:36, power:24, lift:0.3, kbm:0.95, kbum:0.95},
            {dur:0.20, key:'s3', fx:580, hit:true, hx:24, hw:58, hh:42, power:58, lift:1.0, kbm:1.1, kbum:1.1}
          ];
          this.cool=3.6;
          this._idx=0; this._t=0; this.animT=0; return;
        }
        if(adx<360){
          ...
          this._seq=[
            ...
            {dur:0.24, key:'uDash', fx:640},
            {dur:0.20, key:'uFin',  fx:0, hit:true, hx:26, hw:64, hh:50, power:120, lift:1.3, kbm:1.2, kbum:1.2}
          ];
          this.cool=7.0;
          this._idx=0; this._t=0; this.animT=0; return;
        }
      ...
      // 近づく/離れる
      if(adx>220){ this.vx = (dx>0? MOVE : -MOVE); } else { this.vx = (Math.sin(performance.now()/300))*60; }
    }
...
class World{
  constructor(assets, canvas, effects){
    ...
    this.obstacles=[];
    this.stageLeft=STAGE_LEFT; this.stageRight=STAGE_RIGHT_DEFAULT;
    this.zoom=1.0;
  }
  setBounds(l, r){ this.stageLeft = l; this.stageRight = Math.max(l+this.gameW, r); }
...
class Stage1 {
  ...
  spawnWaruGroup(n){
    for(let i=0;i<n;i++){
      ...
      this.g.enemies.push(new WaruMOB(this.g.world, this.g.effects, this.g.assets, x));
    }
  }
  randomSpawnX(){
    const margin=80;
  ...
  update(dt){
    if(this.boss && !this._bossDropAnnounced){
      ...
      if(onGround){
        this._bossDropAnnounced=true;
        this.g.effects.shake(0.35,14);
...
class Game{
  constructor(){
    ...
    // Startボタン & オーバーレイ全体クリックでも開始
    const startBtn=document.getElementById('startBtn');
    const overlay=document.getElementById('titleOverlay');
    startBtn?.addEventListener('click', ()=> this.beginStage1());
    overlay?.addEventListener('pointerdown', (e)=>{ if(e.target.id==='titleOverlay') this.beginStage1(); });
  }
  async start(){
    const imgs=[
      /* 背景 */
      ...
      'B1.png','B2.png','B3.png','B5.png','B6.png','B8.png','B9.png','B10.png','B11.png','B12.png','B13.png','B14.png'
    ];
    ...
    // Startボタンが無い/失敗時の保険：ロード後3秒で自動開始（タイトルに居続ける場合のみ）
    setTimeout(()=>{ if(this.state==='title'){ this.beginStage1(); } }, 3000);
  }
  ...
  _tick(){
        ...
        for(const e of this.enemies){
          if(!p.dead && !e.dead && rectsOverlap(p.aabb(), e.aabb())){
            ...
            if(p instanceof UltBlast){
              const hit=e.hurt(p.power, dir, {lift:1.4,kbMul:3.0,kbuMul:2.8, tag:'ult', spin:true}, this.effects);
              if(hit) this.effects.addSpark(e.x, e.y-10, true);
            }else{
              const hit=e.hurt(p.power, dir, {lift:0.4,kbMul:1.2,kbuMul:1.2, tag:'skill', spin:true}, this.effects);
    ...
    if(this.player.dead && this.player.fade<=0){
      ...
      if(this.player.lives>=0){
        this.player._respawn(this.world);
      }else{
        showBanner('GAME OVER', 1600);
        setTimeout(()=> this.toTitle(), 1600);
      }
    }

    // キルカウント→湧き制御/セクション制御
    if(this.stage) this.stage.onEnemyDeaths();
 ...
 * Boot
 * ========================================= */
const game = new Game(); window.__GAME__ = game; // フェイルセーフ起動用に公開
game.start();
