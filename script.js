// Stage1 Build — Scene-based game with platform & spawn control
(function(){
'use strict';
...
class Assets{
  ...
  load(srcs){
    return Promise.all(srcs.map(src=>new Promise((resolve)=>{
      ...
      img.onerror=()=>{ this.missing.add(src); resolve(); };
      img.src=src;
    })));
  }
...
class Input{
  ...
  _initKeyboard(){
    const down=(k)=>{ if(k==='ArrowLeft'||k==='a'||k==='A') this.left=1;
    ...
    const up=(k)=>{ if(k==='ArrowLeft'||k==='a'||k==='A') this.left=(this.right?1:0);
    ...
    addEventListener('keydown',e=>{down(e.key);},{passive:false});
    addEventListener('keyup',e=>{up(e.key);},{passive:false});
  }
  _initTouch(){
...
class World{
  constructor(assets, canvas, effects){
    ...
    this.bgImg=null; this.bgScale=1; this.bgDW=this.gameW; this.bgDH=this.gameH;
    this.bgRepeat=true;

    // surfaces: [{type:'ground', y:..., left:..., right:...}, {type:'platform', x1,x2,y,cap,occupants:Set}]
    this.surfaces=[];
    this.stageLeft = 0;
    this.stageRight = DEFAULT_STAGE_W;

    // bullets shared (for player skills)
    this._skillBullets=[];
  }
  ...
  setBackground(name, {repeat=true}={}){
    if(this.assets.has(name)){ this.bgImg=this.assets.img(name); this.bgScale = this.gameH / this.bgImg.height;
      this.bgDW = this.bgImg.width*this.bgScale; this.bgDH=this.gameH; this.bgRepeat=repeat;
      // ステージ幅：背景幅（非ループならその幅、ループなら既定幅）
      if(!repeat){ this.stageRight = Math.max(this.gameW, Math.round(this.bgDW)); }
      else{ this.stageRight = Math.max(DEFAULT_STAGE_W, Math.round(this.bgDW)); }
    }else{ this.bgImg=null; this.bgRepeat=true; this.stageRight=DEFAULT_STAGE_W; }
  }
  setSurfaces(list){ this.surfaces=list||[]; }
  getGroundFor(entityX, prevSurface=null, wantPlatformCheck=true){
    // 返す: {y, surface or null}
    let best = {y: this.gameH, surface:null};
    for(const s of this.surfaces){
      if(s.type==='ground'){
        if(entityX>=s.left && entityX<=s.right){
          if(s.y < best.y) best={y:s.y, surface:s};
        }
      } else if(wantPlatformCheck && s.type==='platform'){
        if(entityX>=s.x1 && entityX<=s.x2){
          // 占有制限：すでにこの面に乗っている者はOK、新規はcapチェック
          const occ = s.occupants?.size||0;
          const cap = s.cap||Infinity;
          const allow = (prevSurface===s) || (occ < cap);
          if(allow && s.y < best.y) best={y:s.y, surface:s};
        }
      }
    }
    if(best.y===this.gameH) return {y: Math.floor(this.gameH*0.75), surface:null}; // fallback
    return best;
  }
  updateSurfaceOccupants(characters){
    // 毎フレーム初期化
    for(const s of this.surfaces){ if(s.type==='platform'){ if(!s.occupants) s.occupants=new Set(); s.occupants.clear(); } }
    for(const c of characters){
      if(c._groundSurface && c._groundSurface.type==='platform'){
        c._groundSurface.occupants.add(c);
      }
    }
  }
  updateCam(p){
    const offs=this.effects.getCamOffset();
    const target=clamp(p.x - this.gameW*0.35 + offs.x, this.stageLeft, Math.max(this.stageLeft, this.stageRight - this.gameW));
  ...
  draw(player, enemies){
    ...
    if(this.bgImg){
      if(this.bgRepeat){
        ...
        const endX = this.camX + this.gameW*1.2 + w;
        for(let x=startX; x<=endX; x+=step){ ctx.drawImage(this.bgImg, 0,0,this.bgImg.width,this.bgImg.height, Math.round(x - this.camX), 0, w, h); }
      }else{
    ...
    // ground line (debug thin)
    for(const s of this.surfaces){ if(s.type==='ground'){ ctx.fillStyle='#0b0f17'; const yTop=Math.floor(s.y); ctx.fillRect(0,yTop-1,this.gameW,1); } }
...
/* =========================================
 * Character Base — 相手回転ぶっ飛び/多面地面対応
 * ========================================= */
class CharacterBase{
  constructor(w,h){
  ...
  hurt(amount, dir, opts={}, effects){
    ...
    // スピン付与（生存中も回転）
    const addSpin = (opts.spin||0) + (isStrong? 10:0) + (amount>=25?6:0);
    this.spinSpeed = clamp(this.spinSpeed + addSpin*(dir>0?-1:1), -32, 32);
    ...
    if(this.hp<=0){
      this.dead=true; this.vx = dir * 540; this.vy = -560; this.spinSpeed = 18*(dir>0?-1:1); this.deathT = 0; this.fade = 1;
    }
    return true;
  }
  ...
  updatePhysics(dt, world){
    ...
    // 壁（見えない）
    const leftBound  = world.stageLeft  + WALL_PAD + this.w*0.4;
    const rightBound = world.stageRight - WALL_PAD - this.w*0.4;
    ...
    // 地面/プラットフォーム
    const g = world.getGroundFor(this.x, this._groundSurface, true);
    const top=Math.floor(g.y);
    if(this.y + this.h/2 >= top + FOOT_PAD && this.vy>=0){
      this.y = top - this.h/2 + FOOT_PAD; this.vy=0; this.onGround=true; this._groundSurface=g.surface;
      // 地面に触れたら回転減衰
      this.spinSpeed *= 0.84;
    } else {
      this.onGround=false; this._groundSurface=null;
    }
    ...
    else if(Math.abs(this.spinSpeed)>0.01){ this.spinAngle += this.spinSpeed*dt; this.spinSpeed *= (this.onGround? 0.93: 0.985); }
...
class GroundSpike extends Projectile{
  constructor(world,x,dir,img){
    super(world,x,Math.floor(world.getGroundFor(x).y)-8,dir,img,80);
  ...
  draw(ctx){
    ...
    ctx.save(); ctx.translate(this.x-this.world.camX, Math.floor(this.world.getGroundFor(this.x).y)-this.world.camY); ctx.scale(1,-1);
...
class Player extends CharacterBase{
  constructor(assets, world, effects){
    ...
    this.x=100; this.y=Math.floor(world.getGroundFor(100).y)-this.h/2+FOOT_PAD;
  ...
  currentHitbox(){
    ...
    if(this.state==='skill' || this.state==='skill2' || this.state==='ult'){
      const W=92,H=68; const x=this.x + this.face*(this.w*0.22);
      return {x, y:this.y, w:W, h:H, power:cur.power||0, dir:this.face, lift:cur.lift||0, kbMul:cur.kbMul||1.9, kbuMul:cur.kbuMul||1.45, tag:cur.tag, spin:cur.spin||12};
    }
    if(cur.kind==='hit' || cur.kind==='sp'){
      // 攻撃中は厚み拡張（すり抜け低減）
      const w=58, h=44, x=this.x + this.face*(this.w*0.3 + w*0.5), y=this.y - 6;
      return {x,y,w,h, power:cur.power||0, dir:this.face, lift:cur.lift||1, kbMul:cur.kbMul||1, kbuMul:cur.kbuMul||1, tag:cur.tag, spin:cur.spin||8};
    }
    return null;
  }
        ...
        for(const e of enemies){
          ...
          if(rectsOverlap({x:hb.x,y:hb.y,w:hb.w,h:hb.h}, e.aabb())){
            ...
            if(hit){
              ...
              const dirX = dx>=0? 1 : -1;
              const push = 12;
              this.x += dirX * push*0.5; e.x -= dirX * push*0.5;
            }
          }
        }
      }
  ...
  _startA1(){
    ...
    const seq=[ {kind:'prep',dur:0.06,frame:'k1prep',fx:80,power:0} ];
    let frame='k1a', power=10, fx=160;
    if(this.comboStep===2){ frame='k1b'; power=14; fx=190; }
    else if(this.comboStep===3){ frame='k1c'; power=18; fx=220; }
    seq.push({kind:'hit',dur:0.18,frame,fx,power, kbMul:1.0, kbuMul:1.0, spin:6});
    this._actionSeq=seq; this._actionIndex=0; this._actionTime=0;
  }
  _startA2(){
    this.state='atk'; this.animT=0;
    this._actionSeq=[
      {kind:'prep',dur:0.08,frame:'k2prep',fx:90,power:0},
      {kind:'hit', dur:0.20,frame:'k2',fx:240,power:24, lift:1.0, kbMul:1.15, kbuMul:1.2, after:'enableChase', spin:10}
    ];
  ...
  _startA2Chase(){
    ...
    const seq=[
      ...
      {kind:'hit', dur:0.22,frame:'k1c',fx:300,power:55, lift:1.0, kbMul:1.2, kbuMul:1.2, tag:'chaseFinisher', spin:14}
    ];
    this._actionSeq=seq; this._actionIndex=0; this._actionTime=0;
  ...
  _startSkill1Release(chargeSec){
    ...
    const base   = 36 + Math.floor(t/0.1)*3; // ←基礎UP
    const kbm  = 1.9 + 0.12*(rounds-2);      // ←KB強化
    const kbum = 1.45 + 0.05*(rounds-2);
    const spin = 12 + 4*(rounds-2);          // ←回転付与
    const frames=this.frames.spin; const seq=[];
    for(let r=0;r<rounds;r++){
      for(let i=0;i<frames.length;i++){
        const pow = base*(i===1?1:0.65); const lift=(i===1?1:0);
        seq.push({kind:'sp',dur:0.06,frame:frames[i],fx:90,power:pow,lift, kbMul:kbm, kbuMul:kbum, tag:'skill', spin});
      }
    }
    this._actionSeq=seq; this._actionIndex=0; this._actionTime=0;
  ...
  _startSkill2(){
    ...
    this._skill2SAT = 1.6;
    this._actionSeq=[
      {kind:'hit', dur:0.12, frame:'y1', fx:30, power:6,  tag:'skill', spin:6},
      {kind:'hit', dur:0.12, frame:'y2', fx:30, power:6,  tag:'skill', spin:6},
      {kind:'hit', dur:0.12, frame:'y3', fx:30, power:6,  tag:'skill', spin:6},
      {kind:'hit', dur:0.12, frame:'y4', fx:0,  power:12, tag:'skill', spin:8},
  ...
  _respawn(world){
    ...
    this.x=clamp(world.camX+80, world.stageLeft+40, world.stageRight-40); this.y=Math.floor(world.getGroundFor(this.x).y)-this.h/2+FOOT_PAD; this.vx=0; this.vy=0;
...
/* =========================================
 * Enemies (WaruMOBのみ使用、他はボス戦まで不要だが保持)
 * ここではWaruMOBとScrew（ボス）だけ利用
 * ================================ */
...
class WaruMOB extends CharacterBase{
  constructor(world,effects,assets,x=520){
    ...
    this.x=x; this.y=Math.floor(world.getGroundFor(x).y)-this.h/2+FOOT_PAD; this.face=-1; this.maxhp=100; this.hp=100;
  ...
  hurt(amount, dir, opts={}, effects){
    opts = {...opts, kbMul:(opts.kbMul??1)*1.25, kbuMul:(opts.kbuMul??1)*1.2, spin:(opts.spin||0)+8};
  ...
  update(dt,player){
    ...
    this.brainT-=dt;
    if(this.brainT<=0){
      this.brainT=0.35+Math.random()*0.2;
...
class Screw extends CharacterBase{
  constructor(world,effects,assets,x=1500){
    ...
    this.x=x; this.y=Math.floor(world.getGroundFor(x).y)-this.h/2+FOOT_PAD; this.face=-1;
    this.maxhp=2000; this.hp=2000; // ★ 指定値
  ...
  hurt(amount, dir, opts={}, effects){
    ...
    if(proc){
      opts={...(opts||{}), kbMul:0.40, kbuMul:0.38, spin:(opts.spin||0)*0.5};
    ...
    if(this.cool<=0){
      if(adx<220){
        this.state='atk'; this._seq=[
          {dur:0.10, key:'a2a', fx:200},
          {dur:0.18, key:'a2b', fx:260, hit:true, hx:22, hw:48, hh:38, power:35, lift:0.6, kbm:1.0, kbum:1.0}
        ];
        this.cool=1.2+Math.random()*0.6; this._idx=0; this._t=0; this.animT=0; return;
      }
      if(adx<360){
        this.state='skill';
        this._seq=[
          {dur:0.40, key:'sPrep', fx:0},
      ...
      if(adx<420){
        ...
        this._seq=[
          ...
          {dur:0.24, key:'uFin',  fx:0, hit:true, hx:26, hw:64, hh:50, power:120, lift:1.4, kbm:1.2, kbum:1.2}
        ];
 ...
 * ========================================= */
class StageManager{
  constructor(game){ this.game=game; this.state='title'; this.data={}; }
  setState(st, payload={}){
    this.state=st; Object.assign(this.data,payload);
    const banner=document.getElementById('banner'); banner.classList.add('hidden'); banner.innerHTML='';
    if(st==='title'){
      this.game.enemies.length=0; this.game.world.setBackground('ST1.png',{repeat:true});
      this._applyEntranceSurfaces(); // 初期地形
      this._placePlayerEntrance(true);
      document.getElementById('overlay').classList.remove('hidden');
    }
    if(st==='entrance'){
      document.getElementById('overlay').classList.add('hidden');
      this.game.enemies.length=0; this.game.effects.shake(0.1,5);
      this.game.killsWaru=0;
      this._applyEntranceSurfaces();
      this._placePlayerEntrance(false);
      this._spawnWaruWaveEntrance();
    }
    if(st==='interior'){
      this.game.enemies.length=0; this.game.killsWaru=0;
      this._applyInteriorSurfaces();
      this._placePlayerInterior();
      this._spawnWaruWaveInterior();
    }
    if(st==='boss-intro'){
      // 演出：シェイク＋テキスト1秒
      this.game.effects.shake(0.25,12);
      showBanner('MOBスクリュー登場！',1000);
      // 1秒後にボス戦へ
      setTimeout(()=>{ this.setState('boss-fight'); }, 1000);
    }
    if(st==='boss-fight'){
      this.game.enemies.length=0;
      this._applyInteriorSurfaces(); // 室内のまま
      this._placePlayerInterior();   // 左端スポーン維持
      const boss = new Screw(this.game.world, this.game.effects, this.game.assets, this.game.world.stageLeft + 260);
      this.game.enemies.push(boss);
    }
    if(st==='clear'){
      // ズーム＋2秒表示→タイトル
      const gwrap=document.querySelector('.gamewrap');
      gwrap.classList.add('zooming');
      showBanner('ステージクリア!!',2000);
      setTimeout(()=>{ gwrap.classList.remove('zooming'); this.setState('title'); }, 2000);
    }
  }

  // ===== Surfaces =====
  _applyEntranceSurfaces(){
    const w=this.game.world;
    w.setBackground('ST1.png',{repeat:true});
    // ★ 赤ラインに合わせて地面Yを少し下げる（以前360が高かった）。調整しやすいよう定数化。
    const ENTRANCE_GROUND_Y = 408; // ←必要なら微調整してOK
    const left=w.stageLeft, right=w.stageRight;
    const surfaces=[
      {type:'ground', y: ENTRANCE_GROUND_Y, left, right},
      // コンテナ（上面可、地面スナップ）
      ...this._containerSurfaces(ENTRANCE_GROUND_Y),
      // 側面小プラットフォーム（占有上限2）
      ...this._sidePlatforms(ENTRANCE_GROUND_Y)
    ];
    w.setSurfaces(surfaces);
  }
  _applyInteriorSurfaces(){
    const w=this.game.world;
    w.setBackground('CS.png',{repeat:false}); // 1枚、非ループ、狭い
    const INTERIOR_GROUND_Y = 420; // 室内は現状維持OKと言われたので低め固定
    const left=w.stageLeft, right=w.stageRight;
    w.setSurfaces([{type:'ground', y:INTERIOR_GROUND_Y, left, right}]);
  }
  _containerSurfaces(groundY){
    // コンテナ1~2個、地面に置く。上に乗れる（上面だけ）
    const imgW=120, imgH=64; // 見た目目安（画像サイズに依らずコリジョンのみ）
    const baseY = groundY - imgH; // 天面Y
    const list=[];
    // 左寄り/右寄りに1つずつ
    const x1 = 520, x2 = 980;
    list.push({type:'platform', x1:x1-60, x2:x1+60, y:baseY, cap:99}); // コンテナは上限無し
    list.push({type:'platform', x1:x2-60, x2:x2+60, y:baseY, cap:99});
    return list;
    // 描画は背景に任せる（「浮いて見える」→Yを地面スナップ済み）
  }
  _sidePlatforms(groundY){
    // 入口の「側面止め」用、小さめ足場。敵も登れるが同時占有2体。
    const y=groundY-110;
    return [
      {type:'platform', x1:260, x2:340, y, cap:2},
      {type:'platform', x1:1320, x2:1400, y, cap:2}
    ];
  }

  // ===== Placement =====
  _placePlayerEntrance(centerCam){
    const w=this.game.world;
    const px = w.stageLeft + 120;
    const gy = w.getGroundFor(px).y;
    this.game.player.x=px; this.game.player.y=gy - this.game.player.h/2 + FOOT_PAD; this.game.player.vx=0; this.game.player.vy=0;
    if(centerCam){ w.camX=0; }
  }
  _placePlayerInterior(){
    const w=this.game.world;
    const px = w.stageLeft + 70;
    const gy = w.getGroundFor(px).y;
    this.game.player.x=px; this.game.player.y=gy - this.game.player.h/2 + FOOT_PAD; this.game.player.vx=0; this.game.player.vy=0;
  }

  // ===== Spawns =====
  _randomSpawnX(avoidX, minDist){
    const w=this.game.world;
    for(let tries=0; tries<20; tries++){
      const x = Math.random()*(w.stageRight - w.stageLeft - 200) + w.stageLeft + 100;
      if(Math.abs(x - avoidX) >= (minDist||180)) return x;
    }
    return clamp(avoidX + (Math.random()<0.5?-1:1)*(minDist||180), w.stageLeft+80, w.stageRight-80);
  }
  _spawnWaruWaveEntrance(){
    const g=this.game, w=g.world;
    const needMore = (g.killsWaru||0) < 15;
    if(!needMore) { this.setState('interior'); return; }
    const count=3;
    for(let i=0;i<count;i++){
      const sx = this._randomSpawnX(g.player.x, 200);
      const e = new WaruMOB(w, g.effects, g.assets, sx);
      g.enemies.push(e);
    }
    // 次の監視ループはGame側で全滅チェック
  }
  _spawnWaruWaveInterior(){
    const g=this.game, w=g.world;
    const needMore = (g.killsWaru||0) < 10;
    if(!needMore){ this.setState('boss-intro'); return; }
    const count=3;
    for(let i=0;i<count;i++){
      const sx = this._randomSpawnX(g.player.x, 200);
      const e = new WaruMOB(w, g.effects, g.assets, sx);
 ...
 * UI helpers
 * ========================================= */
function showBanner(text,durMs=1000){
  const b=document.getElementById('banner');
  b.innerHTML=`<div>${text}</div>`; b.classList.remove('hidden');
  setTimeout(()=>{ b.classList.add('hidden'); }, durMs);
}
...
class Game{
  ...
  async start(){
    ...
    this.stage.setState('title');

    const startBtn=document.getElementById('startBtn');
    startBtn.onclick=()=>{ this.stage.setState('entrance'); };
    document.getElementById('overlay').addEventListener('click',(e)=>{
      if(e.target.id==='overlay') this.stage.setState('entrance');
    });
  ...
  _tick(){
      ...
      for(const p of this.world._skillBullets){
        ...
        for(const e of this.enemies){
          if(!p.dead && !e.dead && rectsOverlap(p.aabb(), e.aabb())){
            ...
            const hit=e.hurt(p.power, dir, {lift:0.3,kbMul:0.9,kbuMul:0.9, tag, spin:10}, this.effects);
    ...
    this.enemies=this.enemies.filter(e=>{
      const gone = (e.dead && e.fade<=0);
      if(gone) removed.push(e);
      return !gone;
    });
    if(removed.length>0){
      for(const e of removed){
        if(e instanceof WaruMOB){
          this.killsWaru = (this.killsWaru||0)+1;
        }
      }
      // 全滅チェック（ステートに応じて次ウェーブ）
      if(this.stage.state==='entrance'){
        const liveWaru = this.enemies.some(e=>e instanceof WaruMOB);
        if(!liveWaru){
          if((this.killsWaru||0) < 15) setTimeout(()=>this.stage._spawnWaruWaveEntrance(), 1200);
          else this.stage.setState('interior');
        }
      }
      if(this.stage.state==='interior'){
        const liveWaru = this.enemies.some(e=>e instanceof WaruMOB);
        if(!liveWaru){
          if((this.killsWaru||0) < 10) setTimeout(()=>this.stage._spawnWaruWaveInterior(), 1200);
          else this.stage.setState('boss-intro');
        }
      }
      if(this.stage.state==='boss-fight'){
        const liveBoss = this.enemies.some(e=>e instanceof Screw);
        if(!liveBoss){
          this.stage.setState('clear');
        }
      }
    }
    ...
    for(const e of this.enemies){
      ...
      if(overlapY < overlapX){
        ...
        const dirX = dx>=0? 1 : -1;
        const push = 14;
        this.player.x += dirX * Math.max(push, overlapX*0.8);
        e.x           -= dirX * Math.max(push*0.6, overlapX*0.4);
        this.player.vx += dirX * 24;
        e.vx          -= dirX * 24;
      }
    }

    // プラットフォーム占有更新（敵が登り過ぎないように毎フレーム）
    this.world.updateSurfaceOccupants([this.player, ...this.enemies]);
